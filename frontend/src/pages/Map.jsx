...
const hasProblemLevel = (loc, field) => {
  return loc[field] === "low" || loc[field] === "none";
};

const offersService = (services, service) => {
  return services.includes(service);
};

const isAvailableToHelp = (loc, services) => {
  const offeredServiceLevels = serviceLevels.filter((item) =>
    offersService(services, item.service)
  );

  return (
    offeredServiceLevels.length > 0 &&
    offeredServiceLevels.every((item) => !hasProblemLevel(loc, item.field))
  );
};

const isInNeed = (loc, services) => {
  return serviceLevels.some((item) => {
    return offersService(services, item.service) && hasProblemLevel(loc, item.field);
  });
};
...
const filterMatchesLocation = (filter, loc) => {
  ...
  const filterRules = {
    available: () => isAvailableToHelp(loc, services),
    "in-need": () => isInNeed(loc, services),
    shelter: () =>
    ...
    community: () =>
      ...
      ["community gathering", "youth programs", "senior programs", "newcomer support"].some((s) =>
        services.includes(s)
      ),
    medical: () => type === "medical support" || services.includes("medical support"),
    pet: () => type === "pet support" || services.includes("pet support"),
    info: () =>
      ["information centre", "disaster support hub"].includes(type) ||
      ["information", "recovery information", "family reunification", "disaster support hub"].some(
...
const getFilters = () => {
  ...
  if (userType === "helper")
    return [
      { value: "in-need", label: "🙏 Needs Help" },
      ...categoryFilters,
    ];
