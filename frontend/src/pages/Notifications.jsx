...
function Notifications() {
  ...
  const currentList =
    activeTab === "my-posts" ? myPostsNotifs : myRepliesNotifs;
  const myPostsCount = myPostsNotifs.reduce(
    (sum, group) => sum + group.replies.length,
    0,
  );
  const myRepliesCount = myRepliesNotifs.reduce(
    (sum, group) => sum + group.replies.length,
    0,
  );
  ...
  return (
    <div className="notifications-page">
      ...
      >
        ...
        >
          Your posts
          <span className="notif-tab-count">{myPostsCount}</span>
        </button>
        <button
          role="tab"
        ...
        >
          Your replies
          <span className="notif-tab-count">{myRepliesCount}</span>
        </button>
      </div>
      ...
      {currentList.length === 0 ? (
        ...
        <div className="notif-list">
          {currentList.map((group) => (
            <div key={`${activeTab}-${group.postId}`} className="notif-group">
              {/* Post header */}
              <button
              ...
              >
                ...
                <div className="notif-group-title">{group.postTitle}</div>
                <div className="notif-group-meta">
                  {group.replies.length}{" "}
                  {group.replies.length === 1 ? "reply" : "replies"}
                </div>
              </button>
